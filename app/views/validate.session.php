...
if (!isset($_SESSION['SessionValidate']) || $_SESSION['SessionValidate'] != 'validated') {
    if (isset($_GET['__captcha']) && $_GET['__captcha'] != '') {

        $recaptcha = new \ReCaptcha\ReCaptcha(ProjectDefines::RecaptchaV2()['SecretKey']);
        $resp = $recaptcha->verify($_GET['__captcha'], $ipaddress);
        if ($resp->isSuccess()) {
            $_SESSION['SessionValidate'] = 'validated';
            sleep(1);
        }
    }
}
...
?>
    ...
    <head>
        ...
        <script type="text/javascript">
            function RCallbackF() {
                window.location = '?__captcha=' + grecaptcha.getResponse();
            };
        </script>
    </head>
                    ...
                    <div class="col-xl-6 col-lg-7 col-sm-12 col-12 fxt-bg-color">
                        <div class="fxt-content">
                            <div class="fxt-header">
                                <a href="javascript:;" class="fxt-logo"><img src="/assets/media/logo-d.png" alt="Logo"></a>
                                <p style="margin-top: 20px;"><?= StaticFunctions::lang('Lütfen Oturumunuzu Doğrulayın.') ?>
                                </p>
                            </div>
                            <div style="text-align: center;margin-top:-10px;" class="fxt-form">
                                <center>
                                    <div class="g-recaptcha" data-callback="RCallbackF" data-sitekey=" <?= ProjectDefines::RecaptchaV2()['SiteKey'] ?>"></div>
                                </center>
                            </div>
                        </div>
                    </div>
